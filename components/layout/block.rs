...
impl BlockFlow {
    ...
    fn assign_inline_position_for_formatting_context<'a>(&mut self,
        ...
        let rect = self.base.floats.available_rect(
            self.base.position.start.b,
            self.fragment.border_box.size.block + self.fragment.margin.inline_start_end(),
        ...
        let available_inline_size = if let Some(rect) = rect {
            ...
            let end_edge = self.base.position.start.i + rect.size.inline;
            if end_edge < content_box.size.inline {
                let clamped_margin_end = max(Au(0), self.fragment.margin.inline_end);
                min(end_edge + clamped_margin_end, content_box.size.inline)
            } else {
                rect.size.inline
            }
        } else {
